...
type State = { cancelled: boolean };

function cancellableSleep(ms: number, state: State): Promise<void> {
...
export async function runOnboarding(): Promise<void> {
	...
	const state: State = { cancelled: false };
	...
	try {
		await cancellableSleep(200, state);
		if (state.cancelled) return;

		floodHint = showHint("flood", "/assets/icons/ui/flood-info.svg");
		await animateSlider(0, 100, 1200, state);
		await cancellableSleep(30, state);
		await animateSlider(100, 0, 1000, state);
		if (state.cancelled) return;
		await hideHint(floodHint);
		floodHint = null;
		await cancellableSleep(100, state);
		if (state.cancelled) return;
		...
		await cancellableSleep(1700, state);
		if (state.cancelled) return;
		...
		if (state.cancelled) {
			const range = document.getElementById("slider-flood_simulation") as HTMLInputElement | null;
			if (range) {
				range.value = "0";
				range.dispatchEvent(new Event("input"));
			}

			const navigationHappened = await new Promise<boolean>((resolve) => {
				const onNav = () => resolve(true);
				document.addEventListener("app-view-updated", onNav, { once: true });
				setTimeout(() => {
					document.removeEventListener("app-view-updated", onNav);
					resolve(false);
				}, 500);
			});
