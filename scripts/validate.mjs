...
try {
	...
	console.error(
		`${RED}[Error] Failed to load context.yaml: ${e.message}${RESET}`,
	);
	process.exit(1);
}
...
try {
	...
	console.error(
		`${YELLOW}[Warning] Failed to load layers.yaml: ${e.message}${RESET}`,
	);
}
...
for (const scenarioDef of scenarios) {
	...
	try {
		...
		console.error(
			`${YELLOW}[Warning] Cannot load ${scenarioFile}: ${e.message}${RESET}`,
		);
		continue;
	}
	...
	for (const role of roles) {
		...
		try {
			...
			errors.push(
				...validateChallengeRelations(challengeFile, rawChallenge, knownLayers),
			);
			errors.push(...validateI18nChallenge(challengeFile, rawChallenge));
		} catch (e) {
			console.error(
				`${YELLOW}[Warning] Cannot load ${challengeFile}: ${e.message}${RESET}`,
			);
		}
	}
}
...
for (const [id, l] of layerMap.entries()) {
	if (l.layer_type === "locations" && l.src) {
		try {
			...
			console.error(
				`${YELLOW}[Warning] Cannot load ${l.src}: ${e.message}${RESET}`,
			);
		}
	}
}
