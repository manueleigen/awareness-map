...
async function getResource(lang, filesLocation) {
  if (cache.has(lang)) return JSON.parse(cache.get(lang));
...
async function getTranslationByKey(lang, key, options) {
  if (!key || typeof key !== "string") throw new Error("Expected a valid string key.");
  const translation = await getResource(lang, options.filesLocation);
...
export function createTranslator(userOptions = {}) {
  ...
  return {
    ...
    getTranslationByKey: (lang, key) => getTranslationByKey(lang, key, options)
  };
}